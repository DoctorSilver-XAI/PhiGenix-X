...
export class OpenAIProvider implements AIService {
    ...
    async initialize(): Promise<void> {
        ...
        if (storedKey) {
            ...
            // Clé par défaut fournie par l'utilisateur
    ...
    configure(apiKey: string) {
        ...
        this.openai = new OpenAI({
            apiKey: apiKey,
            dangerouslyAllowBrowser: true // Usage client-side assumé pour ce projet perso
        });
    ...
    async generateResponse(messages: AIChatMessage[], modelId: string, _options?: { conversationId?: string }): Promise<AIResponse> {
        ...
        // Support spécifique pour l'endpoint GPT-5 (responses.create)
        if (modelId.startsWith('gpt-5') || modelId === 'gpt-5.2') {
            try {
                // @ts-ignore - Nouvelle API non encore typée
    ...
    async streamResponse(messages: AIChatMessage[], modelId: string, onChunk: (chunk: string) => void, _options?: { conversationId?: string }): Promise<void> {
        ...
        if (modelId.startsWith('gpt-5') || modelId === 'gpt-5.2') {
            try {
                // @ts-ignore - Nouvelle API streaming
                ...
                // Fallback : Streaming standard OpenAI (au lieu de bloquant)
                // Cela assure la fluidité même si l'endpoint expérimental échoue
