...
export function ChatInterface() {
        ...
        try {
            ...
            if (res.ok) {
                ...
                if (data.length === 0) {
                    ...
                    const mostRecent = data[0]; // Already sorted by backend
    ...
    const fetchFullSession = async (id: string) => {
        try {
            ...
            if (res.ok) {
                ...
                setSessions(prev => prev.map(s => s.id === id ? detailedSession : s));
            }
        } catch (e) {
    ...
    const createNewSession = async () => {
        try {
            ...
            if (res.ok) {
                ...
                // Initialize with an empty message array if checking length later
                newSession.messages = [{
                    id: 'init',
                    role: 'assistant',
    ...
    const handleSendMessage = async (e?: React.FormEvent) => {
        ...
        try {
            ...
            // After stream is done, we might want to refresh the session title if it changed on backend
            // (The orchestrator auto-updates title).
            // We can do a lazy re-fetch after a short delay
    ...
    const handleClearCurrent = () => {
        // With backend, we probably should just delete the session and start a new one?
        // Or implement a clear endpoint. For now, let's just delete and recreate to be clean.
        if (currentSessionId) {
